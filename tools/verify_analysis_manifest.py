#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Verify artifact manifest for outputs/analysis."""
...
def infer_root_from_rows(rows: list[dict[str, str]]) -> Path:
    if not rows:
        return Path("outputs/analysis").resolve()

    parts = rows[0]["path"].split("/")
    if len(parts) >= 2:
        return (Path(parts[0]) / parts[1]).resolve()
    return Path("outputs/analysis").resolve()
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Verify curated analysis artifact manifest.")
    ap.add_argument("--manifest", type=str, default="outputs/analysis/ARTIFACT_MANIFEST.csv")
    ...
    for row in rows:
        ...
        got_size = str(path.stat().st_size)
