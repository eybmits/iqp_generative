...
- experiment: `Experiment 14 hardware recovery curve`
...
- experiment-specific metric note: Stores expected recovery fraction R(Q) over the elite-unseen state set for target, uniform, spectral completion, and IQP simulation/hardware curves.

- note: Build a single-seed recovery curve from Experiment 13 hardware counts using the same elite-unseen set as in Experiment 4, together with simulation curves for the same fixed `(sigma, K)` setting.

- source driver: `experiment_14_hardware_recovery_curve.py`
