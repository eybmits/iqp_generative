#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Experiment 2: standalone beta-vs-KL summary plot.

This script is intentionally standalone. It does not import any other repo
scripts and can either:

1. read the existing aggregated KL metrics CSV from the beta sweep, or
2. rerender directly from a saved normalized series CSV.

It renders exactly one PDF panel in the established final-figure style.
"""
...
DEFAULT_METRICS_CSV = ROOT / "plots" / "experiment_2_beta_kl_summary" / "experiment_2_beta_kl_summary_series.csv"
DEFAULT_SERIES_CSV = ROOT / "plots" / "experiment_2_beta_kl_summary" / "experiment_2_beta_kl_summary_series.csv"
DEFAULT_OUTDIR = ROOT / "plots" / "experiment_2_beta_kl_summary"
STEM = "experiment_2_beta_kl_summary"
...
def _load_series_from_metrics_csv(metrics_csv: Path) -> List[Dict[str, object]]:
    ...
    with metrics_csv.open("r", encoding="utf-8", newline="") as f:
        ...
        for row in reader:
            ...
            if str(row["metric"]) != "KL_pstar_to_q":
                continue
    ...
    if not rows_out:
        raise ValueError(f"No KL rows found in {metrics_csv}")
    return rows_out
...
def render_plot(series_rows: Sequence[Dict[str, object]], out_pdf: Path) -> None:
    apply_final_style()
    ...
    ax.set_ylabel(r"$D_{\mathrm{KL}}(p^{*}\,\|\,q)$")
...
def run() -> None:
    ap = argparse.ArgumentParser(description="Experiment 2: standalone beta-vs-KL summary plot.")
    ap.add_argument("--metrics-csv", type=str, default=str(DEFAULT_METRICS_CSV))
    ap.add_argument("--series-csv", type=str, default=str(DEFAULT_SERIES_CSV))
    ap.add_argument("--outdir", type=str, default=str(DEFAULT_OUTDIR))
    ap.add_argument("--stem", type=str, default=STEM)
    args = ap.parse_args()

    metrics_csv = Path(args.metrics_csv)
    series_csv_arg = str(args.series_csv).strip()
    outdir = Path(args.outdir)
    outdir.mkdir(parents=True, exist_ok=True)

    if series_csv_arg:
        input_mode = "series_csv"
        source_path = Path(series_csv_arg)
    ...
    render_plot(series_rows, out_pdf)
    ...
    _write_json(
        outdir / "RERENDER_CONFIG.json",
        {
            ...
            "rerender_command": (
                ...
                f"--series-csv {_try_rel(out_series_csv)} --outdir {_try_rel(outdir)} --stem {stem}"
            ),
        },
    )
