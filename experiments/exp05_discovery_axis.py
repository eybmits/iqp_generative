...
def _plot_q80_vs_k(pair_rows: List[Dict], Ks: List[int], train_m: int, layers: int, outdir: str) -> None:
    ...
    ax.plot(ks_sorted, mean_prob, color=COLOR_PROB, linewidth=2.0, label="prob MSE (data-fit)")
...
def _plot_recovery_curves_all(
    ...
    loss_handles = [
        ...
        Line2D([0], [0], color=COLOR_PROB, lw=2, label="prob MSE (data-fit)"),
    ]
    if include_target:
...
def main() -> None:
    ...
    if args.target_family == "paper":
        p_star, support, scores = hv.build_target_distribution_paper(args.n, args.beta)
    else:
        p_star, support, scores = hv.build_target_distribution_iqp_hard(
            n=args.n,
            depth=args.target_iqp_depth,
            seed=args.target_iqp_seed,
            scale=args.target_iqp_scale,
            temperature=args.target_iqp_temperature,
            even_parity_only=bool(args.target_iqp_even_parity),
            outdir=None,
        )
    ...
    for seed in seeds:
        ...
        for train_m in train_ms:
            ...
            for layers in layers_list:
                # Train prob-MSE model once per (seed, train_m, layers)
                cfg_prob = hv.Config(
                    n=args.n,
                    beta=args.beta,
                    train_m=train_m,
                ...
                art_prob = hv.rerun_single_setting(
                    ...
                    sigma=float(args.sigma), K=int(min_K),
                    return_hist=False, iqp_loss="prob_mse"
                )
                q_prob = art_prob["q_iqp"]
                metrics_prob = hv.compute_metrics_for_q(
                    q_prob, holdout_mask, qH_unif, H_size,
                    args.Q80_thr, args.Q80_search_max
                )
                ...
                for K in Ks:
                    ...
                    art_parity = hv.rerun_single_setting(
                        ...
                        sigma=float(args.sigma), K=int(K),
                        return_hist=False, iqp_loss="parity_mse"
                    )

                    metrics_parity = hv.compute_metrics_for_q(
                        art_parity["q_iqp"], holdout_mask, qH_unif, H_size,
                        args.Q80_thr, args.Q80_search_max
                    )

                    # Fit metrics to training distribution
                    ...
                    results_pair.append({
                        ...
                        "Q80_ratio_parity_over_prob": float(metrics_parity["Q80"] / metrics_prob["Q80"])
                        if metrics_prob["Q80"] > 0 else float("nan"),
                    ...
                    if bool(args.plot_recovery):
                        y_parity = hv.expected_unique_fraction(art_parity["q_iqp"], holdout_mask, Q_grid)
