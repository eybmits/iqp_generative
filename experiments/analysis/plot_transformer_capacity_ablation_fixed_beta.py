#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Fixed-beta Transformer capacity ablation against IQP parity.
...
from plot_fig6_beta_sweep_recovery_grid_multiseed import (  # noqa: E402
    HAS_PENNYLANE,
    HAS_TORCH,
    _ARTransformer,
...
OUTPUT_STEM = "transformer_capacity_ablation_fixed_beta"
PNG_DPI = 300
    ...
    *,
    ...
    def _draw_capacity_panel(ax, mean_key: str, std_key: str, seed_key: str, ylabel: str) -> None:
        ...
        for variant_row in transformer_summary:
            ...
            ax.scatter(x_jitter, vals, color="#7FB3E6", alpha=0.45, s=20, linewidths=0.0, zorder=6)
        ...
        if iqp_summary is not None:
            ...
            ax.text(iqp_x * 1.15, iqp_y, "IQP parity", fontsize=8.0, color="#C40000", va="center")
...
def run() -> None:
    ap = argparse.ArgumentParser(description="Fixed-beta Transformer capacity ablation against IQP parity.")
    ap.add_argument(
        "--outdir",
        type=str,
        default=str(ROOT / "outputs" / "analysis" / "transformer_capacity_ablation_beta0p90"),
    )
    ...
    _write_json(
        run_config_json,
        {
            "script": "experiments/analysis/plot_transformer_capacity_ablation_fixed_beta.py",
            ...
            "rerun_command": (
                ...
                "experiments/analysis/plot_transformer_capacity_ablation_fixed_beta.py "
    ...
    write_training_protocol(
        outdir,
        experiment_name="Transformer capacity ablation at fixed beta",
        note="This capacity ablation uses the shared 10-seed / 600-budget analysis standard.",
        source_relpath="experiments/analysis/plot_transformer_capacity_ablation_fixed_beta.py",
