#!/usr/bin/env python3
"""Experiment 13: IBM hardware sampling for global-best IQP parity vs IQP MSE coverage."""
...
OUTPUT_STEM = "experiment_13_ibm_hardware_global_best_coverage"
DEFAULT_OUTDIR = ROOT / "plots" / "experiment_13_ibm_hardware_global_best_coverage"
...
def _build_model_weights_by_key(exp12_data: np.lib.npyio.NpzFile) -> Dict[str, np.ndarray]:
    return {
        "parity_global_best": np.asarray(exp12_data["global_best_parity_weights"], dtype=np.float64),
...
def _render_summary_plot(
    *,
    ...
    parity_rows = {int(row["seed"]): row for row in per_seed_rows if row["model_key"] == "parity_global_best"}
    ...
    ax_seed.scatter(
        ...
        y,
        s=54,
        color=MODEL_COLORS["parity_global_best"],
        marker=MODEL_MARKERS["parity_global_best"],
        label=MODEL_LABELS["parity_global_best"],
        zorder=3,
    )
...
def _write_readme(
    *,
    ...
    lines = [
        ...
        f"- global-best `(sigma, K)`: `({float(summary_payload['global_best_sigma']):g}, {int(summary_payload['global_best_k'])})`",
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Experiment 13: IBM hardware sampling for global-best IQP coverage.")
    ...
    if not HAS_QISKIT:
        raise RuntimeError("Qiskit and qiskit-ibm-runtime are required for Experiment 13.")
    ...
    if args.dry_run:
        dry_payload = {
            ...
            "train_m": train_m,
            "seeds": seeds.tolist(),
            "global_best_sigma": global_best_sigma,
    ...
    parity_rows = [row for row in per_seed_rows if row["model_key"] == "parity_global_best"]
    ...
    write_training_protocol(
        outdir,
        experiment_name="Experiment 13 IBM hardware global-best IQP coverage",
        note=(
            "Load Experiment 12 global-best parity weights and IQP-MSE weights, sample the final circuits on IBM hardware, "
