...
This section addresses the core operational question of the paper: under strict holdout masking, how many model samples are required to rediscover unseen states.
Our methodology follows a fixed three-stage pipeline:
\emph{(i)} construct target and holdout protocols,
\emph{(ii)} train IQP and classical models under matched data/budget conditions with parity moment-matching supervision,
and \emph{(iii)} evaluate discovery in sampling units via \(R(Q)\), \(Q_{80}\), and budget-law diagnostics.
Result-level visual summaries of these components are presented in the corresponding Results figures.
...
Primary supervision is parity moment matching, implemented as parity-moment MSE (\texttt{parity\_mse}).
Additional IQP objectives (\texttt{prob\_mse}, \texttt{xent}, and a kernelized moment-matching variant, code option \texttt{mmd}) are evaluated in dedicated analyses.

\paragraph{Classical baselines.}
We include:
\begin{enumerate}
\item Ising+fields (NN+NNN), parity moment-matching objective.
...
\paragraph{Parity moment-matching supervision and empirical moments.}
...
\end{equation}
The lower bound corresponds to the best-case regime where model-assigned holdout mass is spread as uniformly as possible inside \(H\);
large gaps between measured \(Q_{80}\) and \(Q_{80}^{\mathrm{lb}}\) indicate within-holdout imbalance (state-level concentration / effective collapse).
...
Reproducible claim-level entry points are in \texttt{experiments/claims/claim01\_...\ claim10\_...}, each writing claim-scoped outputs under \texttt{outputs/claims/}.
